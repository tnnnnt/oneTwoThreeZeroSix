...
for data_query_g in datas_query_g:
	...
	if has_tickets == 'Y':
		...
		print('车次: {}, 票面出发站: {}, 票面到达站: {}, 实际到达站: {}, 出发时间: {}, 到达时间: {}, 历时: {}'.format(
		...
		try:
			...
			for from_ind in range(from_station_ind + 1):
				...
				for to_ind in range(from_station_ind + 1, len(station_names)):
					...
					possible_train_datas[station_names[from_ind]][station_names[to_ind]][train_no].append(to_station)
		except ValueError:
			print('------')
			print("无法解析 JSON 响应")
			print(tmp)
...
for from_station_in_ticket, v in possible_train_datas.items():
	...
	for to_station_in_ticket, vv in v.items():
		...
		params_query_g['leftTicketDTO.from_station'] = from_station_in_ticket_telecode
		params_query_g['leftTicketDTO.to_station'] = to_station_in_ticket_telecode
		chance_no = 1
		while True:
			try:
				response = requests.get(url_query_g, headers=headers, params=params_query_g, cookies=cookies)
		...
		for data_query_g_tmp in datas_query_g_tmp:
			...
			ticket_stock_and_price = tmp[39]
			print('车次: {}, 票面出发站: {}, 票面到达站: {}, 实际到达站: {}, 出发时间: {}, 到达时间: {}, 历时: {}'.format(
				train_number, from_station_in_ticket, to_station_in_ticket, vv[train_no], start_time, end_time, duration_time))
			for i in range(0, len(ticket_stock_and_price), 10):
				t = ticket_stock_and_price[i:i+10]
				if t[7:] != '000':
					print(seat_type[t[0]], end='')
